...
class Topology(NoviObject):
    ...
    def serialize(self, name):
        graph = generateGraph()
        self.toRDF(graph)
        graph.serialize(destination="%s.n3"% name,format="n3")
        graph.serialize(destination="%s.owl"% name,format="xml")
        
    def toRDF(self, graph):
        super(Topology, self).toRDF(graph)
...
class Node(NoviObject):
    ...
    def toRDF(self, graph):
        ...
        if self._implementedBy:
            ...
            self._implementedBy.toRDF(graph)
        if self._hostname:
...
class ImplNode(Node):
    ...
    def toRDF(self, graph):
        super(ImplNode,self).toRDF(graph)
...
class Link(NoviObject):
    ...
    def toRDF(self, graph):
        ...
        if self._provisionedBy:
            ...
            self._provisionedBy.toRDF(graph)
        else:
...
class Interface(NoviObject):
    ...
    def toRDF(self, graph):
        super(Interface,self).toRDF(graph)
...
class NSwitch(NoviObject):
    ...
    def toRDF(self, graph):
        super(NSwitch,self).toRDF(graph)
...
def generateBoundTopology():
    ...
    topo.addLinks(links)
    topo.serialize("bound-request")
    
def generateUnboundTopology():
    topDict = {}
    for x in ["lrouter","lrouter2","sliver1","sliver2","vm1"]:
        topDict[x] = Node(x)
    topDict["lrouter"].setPrefixName("if")
    ...
    links.extend(connect(topDict["lrouter"],topDict["sliver1"],
            ...
            ipY="192.168.37.3",netmaskY="255.255.255.248"))
    links.extend(connect(topDict["lrouter2"],topDict["vm1"]))
    topo = Topology("UnboundTopology")
    topo.addNodes(topDict.values())
    topo.addLinks(links)
    topo.serialize("unbound-request")
    
def main():
    generateBoundTopology()
    print "Created/overwritten bound-request.n3 and bound-request.owl"
    generateUnboundTopology()
    print "Created/overwritten bound-request.n3 and bound-request.owl"
